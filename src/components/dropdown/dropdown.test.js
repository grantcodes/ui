...
describe("Dropdown Component", () => {
	...
	it("should have menu slot", async () => {
		...
		assert.ok(slot, "Menu slot should exist when open");
	});

	it("should not render menu when closed", async () => {
		element = await fixture("grantcodes-dropdown");
		const menu = element.shadowRoot.querySelector(".dropdown__menu");
		assert.ok(!menu, "Menu should not be rendered when closed");
	});
	...
	it("should render menu when open", async () => {
		...
		const menu = element.shadowRoot.querySelector(".dropdown__menu");
		assert.ok(menu, "Menu should be rendered when open");
	});
	...
	it("should toggle open state when trigger is clicked", async () => {
		...
		assert.strictEqual(element.open, false, "Should start closed");

		click(trigger);
		await element.updateComplete;

		assert.strictEqual(element.open, true, "Should be open after click");
	});
	...
	it("should emit toggle event when opened/closed", async () => {
		element = await fixture("grantcodes-dropdown");
